...
class Dataset(object):
    ...
    def load(self, verbose, scale=None):
        ...
        self.train_pos, self.train_neg, select = get_gt(self.q_train, self.db_train, self.intra_thres, self.inter_thres)
        ...
        self.val_pos, select = get_gt(self.q_val, self.db_val, 25, None)
        assert(len(select)==len(self.q_val))
        self.test_pos, select = get_gt(self.q_test, self.db_test, 25, None)
...
def get_gt(query, gallery, intra_thres, inter_thres=None):
