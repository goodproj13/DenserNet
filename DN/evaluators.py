...
def get_pairwise_distance(features, query=None, gallery=None, metric=None):
...
def feature_extraction(model, data_loader, dataset, print_freq=10,
    ...
    with torch.no_grad():
        for i, (imgs, fnames, _, _, _) in enumerate(tqdm(data_loader, desc="Extract Features (GPU-%d)" % gpu)):
            ...
            outputs = get_cnn_feature(model, imgs, vlad, gpu=gpu)
            if (pca is not None):
...
def get_cnn_feature(model, inputs, vlad=True, gpu=None):
    model.eval()
...
def get_recalls(distmat, gt, gallery, recall_topk=[1, 5, 10], nms=False):
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, dataset, query, gallery, ground_truth, gallery_loader=None, \
        ...
        if (gallery_loader is not None):
            features = feature_extraction(self.model, query_loader, query,
                                        vlad=vlad, pca=pca, gpu=gpu, sync_gather=sync_gather)
            features_db = feature_extraction(self.model, gallery_loader, gallery,
                                        vlad=vlad, pca=pca, gpu=gpu, sync_gather=sync_gather)
            features.update(features_db)
        else:
            features = feature_extraction(self.model, query_loader, dataset,
                            vlad=vlad, pca=pca, gpu=gpu, sync_gather=sync_gather)

        distmat, _, _ = get_pairwise_distance(features, query, gallery)
        recalls = get_recalls(distmat, ground_truth, gallery, nms=nms)
        if (not rerank):
            return recalls

        if (self.rank==0):
            print('Applying re-ranking ...')
            distmat_gg, _, _ = get_pairwise_distance(features, gallery, gallery)
            distmat_qq, _, _ = get_pairwise_distance(features, query, query)
            distmat = re_ranking(distmat.numpy(), distmat_qq.numpy(), distmat_gg.numpy(),
                                k1=rr_topk, k2=1, lambda_value=lambda_value)

        return get_recalls(distmat, ground_truth, gallery, nms=nms)
